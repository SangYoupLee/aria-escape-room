    ...
    <div className="w-full rounded border overflow-hidden"
      ...
      <div className="flex gap-2 p-3 border-b" style={{ borderColor: 'var(--border)' }}>
        {[
          ...
          <button key={key} onClick={() => setActiveTab(key)}
            className="flex-1 mono font-semibold py-3 rounded transition-colors"
            style={{
              fontSize: '15px',
      ...
      {activeTab === 'log' && (
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-52 flex-shrink-0 p-4 flex flex-row md:flex-col gap-3
                          border-b md:border-b-0 md:border-r"
            style={{ borderColor: 'var(--border)' }}>

            <div className="flex-1 rounded border p-3"
              style={{ borderColor: 'var(--border)', background: 'rgba(0,0,0,0.25)' }}>
              <p className="mono text-xs mb-2" style={{ color: 'var(--cyan)' }}>[ 경로 요약 ]</p>
              <div className="flex flex-col gap-1">
                {[
                  { label: 'START', value: 'ZONE 01' },
                  { label: 'END',   value: 'ZONE 06' },
                ].map(({ label, value }) => (
                  <div key={label} className="mono text-xs flex gap-2"
            ...
            <div className="flex-1 rounded border p-3"
              style={{ borderColor: 'var(--border)', background: 'rgba(0,0,0,0.25)' }}>
              <p className="mono text-xs mb-2" style={{ color: 'var(--cyan)' }}>[ 연구원 메모 ]</p>
              ...
              {LOG_ENTRIES.map(({ id, time, zone }) => (
                <div key={id} className="rounded border p-3"
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="mono text-xs" style={{ color: 'var(--text-secondary)' }}>{time}</span>
                  </div>
                  <div className="mono font-bold text-center py-1"
                    style={{ fontSize: '16px', color: 'var(--text-primary)', letterSpacing: '0.08em' }}>
                    ZONE {zone}
                  </div>
                </div>
              ))}
        ...
        <div className="p-3">
          <div className="flex flex-wrap justify-between items-center gap-2 mb-3">
            ...
            <div className="flex gap-3">
              <span className="mono text-xs" style={{ color: 'var(--text-secondary)' }}>ZONE 01 — START</span>
              <span className="mono text-xs" style={{ color: 'var(--text-secondary)' }}>ZONE 06 — END</span>
            </div>
          </div>
